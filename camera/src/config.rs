...
impl Config {
    ...
    pub fn generate_record_cmd(&self, file_pattern: &str, start_number: usize) -> String {
        ...
        format!(
            "{} -t 0 --width {} --height {} --framerate {} --bitrate {} --nopreview --inline --intra {} --output -{} | ffmpeg -hide_banner -loglevel error -nostats -threads 1 -y -fflags +genpts -f h264 -r {} -i - -c:v copy -f segment -segment_time {} -segment_start_number {} -segment_format mp4 -segment_format_options movflags=+frag_keyframe+empty_moov+default_base_moof -reset_timestamps 1 \"{}\"",
            self.cam_cmd,
            self.width,
            self.height,
            self.framerate,
